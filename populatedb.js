...
const Child_Category = require("./models/child_category");
const Items = require("./models/items");

const categorys = [];
const child_categorys = [];
const items = [];
...
async function childCategoryCreate(name, category) {
  childCategoryDetail = { name: name, category: category };

  const childCategory = new Child_Category(childCategoryDetail);

  await childCategory.save();
  child_categorys.push(childCategory);
  console.log(`Added child_category: ${name}`);
}

async function itemsCreate(name, quantity, price, childOf) {
  itemDetail = {
    name: name,
    quantity: quantity,
    price: price,
...
async function createCategory() {
  ...
  await Promise.all([
    categoryCreate("Aluminium"),
    categoryCreate("Brass"),
    categoryCreate("Copper"),
  ]);
}

async function createChildCategory() {
  console.log("Adding childCategories");
  await Promise.all([
    childCategoryCreate("Aluminium Rod", categorys[0]),
    childCategoryCreate("Aluminium Flat", categorys[0]),
    childCategoryCreate("Brass Rod", categorys[1]),
  ]);
}

async function createItems() {
  console.log("Adding Items");
  await Promise.all([itemsCreate("32 dia", "50", "230", child_categorys[0])]);
}
